...
contract Governance is GovernanceConstants {
    ...
    function addToken(address _token) external {
        require(msg.sender == address(tokenGovernance), "5a");
        require(tokenIds[_token] == 0, "5b"); // token exists
        require(totalTokens < MAX_AMOUNT_OF_REGISTERED_TOKENS, "5c"); // no free identifiers for tokens
    ...
    function requireGovernor(address _address) public view {
        require(_address == networkGovernor, "5d"); // only by governor
    }

    /// @notice Checks if validator is active
    /// @param _address Validator address
    function requireActiveValidator(address _address) external view {
        require(validators[_address], "5e"); // validator is not active
    }
    ...
    function validateTokenAddress(address _tokenAddr) external view returns (uint16) {
        ...
        require(tokenId != 0, "5f"); // 0 is not a valid token
        return tokenId;
    }
    ...
    function packRegisterNFTFactoryMsg(
        ...
        return
            abi.encodePacked(
                ...
                "\nCreator's account ID in vengine: ",
    ...
    /// @param _creatorAccountId Creator's vengine account ID
    ...
    function registerNFTFactoryCreator(
        ...
        require(address(nftFactories[_creatorAccountId][_creatorAddress]) == address(0), "5g");
        ...
        require(recoveredAddress == _creatorAddress && recoveredAddress != address(0), "5h");
    ...
    function setDefaultNFTFactory(address _factory) external {
        requireGovernor(msg.sender);
        require(address(_factory) != address(0), "5i"); // Factory should be non zero
        require(address(defaultFactory) == address(0), "5j"); // NFTFactory is already set
    ...
    function getNFTFactory(uint32 _creatorAccountId, address _creatorAddress) external view returns (NFTFactory) {
        ...
        if (address(_factory) == address(0)) {
            require(address(defaultFactory) != address(0), "5k"); // NFTFactory does not set
