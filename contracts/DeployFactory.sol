...
contract DeployFactory is TokenDeployInit {
    ...
    constructor(
        ...
        deployProxyContracts(_govTarget, _verifierTarget, _vengineTarget, _genesisRoot, _firstValidator, _governor);

        selfdestruct(msg.sender);
    }

    event Addresses(address governance, address vengine, address verifier, address gatekeeper);
    ...
    function deployProxyContracts(
        ...
        AdditionalVengine additionalVengine = new AdditionalVengine();
        Proxy vengine = new Proxy(
            address(_vengineTarget),
            abi.encode(address(governance), address(verifier), address(additionalVengine), _genesisRoot)
        );

        UpgradeGatekeeper upgradeGatekeeper = new UpgradeGatekeeper(vengine);
        ...
        vengine.transferMastership(address(upgradeGatekeeper));
        upgradeGatekeeper.addUpgradeable(address(vengine));

        upgradeGatekeeper.transferMastership(_governor);

        emit Addresses(address(governance), address(vengine), address(verifier), address(upgradeGatekeeper));
