...
contract RegenesisMultisig is Ownable, Constants {
    ...
    function submitHash(bytes32 _oldRootHash, bytes32 _newRootHash) external {
        // Only vengine team can submit the hashes
        require(msg.sender == getMaster(), "1a");
    ...
    function approveHash(bytes32 _oldRootHash, bytes32 _newRootHash) external {
        require(_oldRootHash == candidateOldRootHash, "1b");
        require(_newRootHash == candidateNewRootHash, "1c");
