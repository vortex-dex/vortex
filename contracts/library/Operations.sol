...
/// @title vengine operations tools
library Operations {
    // Circuit ops and their pubdata (chunks * bytes)

    /// @notice vengine circuit operation type
    enum OpType {
        Noop,
        Deposit,
        TransferToNewAddr,
    ...
    /// @dev vengine account id bytes lengths
    uint8 internal constant ACCOUNT_ID_BYTES = 4;
    /// @dev vengine nft serial id bytes lengths
    uint8 internal constant NFT_SERIAL_ID_BYTES = 4;
    uint8 internal constant AMOUNT_BYTES = 16;
    /// @dev Signature (for example full withdraw signature) bytes length
    ...
    uint256 public constant PACKED_FULL_WITHDRAW_PUBDATA_BYTES =
        OP_TYPE_BYTES +
            ACCOUNT_ID_BYTES +
            ADDRESS_BYTES +
            TOKEN_BYTES +
    ...
    function readFullWithdrawPubdata(bytes memory _data) internal pure returns (FullWithdraw memory parsed) {
        // NOTE: there is no check that variable sizes are same as constants (i.e. TOKEN_BYTES), fix if possible.
        uint256 offset = OP_TYPE_BYTES;
        (offset, parsed.accountId) = Bytes.readUInt32(_data, offset); // accountId
        (offset, parsed.owner) = Bytes.readAddress(_data, offset); // owner
        ...
        require(offset == PACKED_FULL_WITHDRAW_PUBDATA_BYTES, "O"); // reading invalid full withdraw pubdata size
    }

    function writeFullWithdrawPubdataForPriorityQueue(FullWithdraw memory op) internal pure returns (bytes memory buf) {
        buf = abi.encodePacked(
            uint8(OpType.FullWithdraw),
    ...
    function checkFullWithdrawInPriorityQueue(FullWithdraw memory op, bytes20 hashedPubdata) internal pure returns (bool) {
        return Utils.hashBytesToBytes20(writeFullWithdrawPubdataForPriorityQueue(op)) == hashedPubdata;
    }
    ...
    function readForcedWithdrawPubdata(bytes memory _data) internal pure returns (ForcedWithdraw memory parsed) {
        // NOTE: there is no check that variable sizes are same as constants (i.e. TOKEN_BYTES), fix if possible.
    ...
    function readUpdatePublicKeyPubdata(bytes memory _data) internal pure returns (UpdatePublicKey memory parsed) {
        uint256 offset = OP_TYPE_BYTES;
        (offset, parsed.accountId) = Bytes.readUInt32(_data, offset); // accountId
