...
/// @title vengine storage contract
contract Storage {
    ...
    /// @dev Flag indicates that a user has exited in the SafeExit mode certain token balance (per account id and tokenId)
    mapping(uint32 => mapping(uint32 => bool)) internal performedSafeExit;

    /// @dev Flag indicates that safe exit (mass exit) mode is triggered
    /// @dev Once it was raised, it can not be cleared again, and all users must exit
    bool public safeExitMode;
    ...
    /// @member commitment Verified input for the vengine circuit
    ...
    AdditionalVengine internal additionalVengine;
    ...
    /// @notice Checks that current state not is SafeExit mode
    function requireActive() internal view {
        require(!safeExitMode, "L"); // SafeExit mode activated
    }
}
