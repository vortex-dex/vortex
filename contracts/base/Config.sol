...
/// @title vengine configuration constants
contract Config {
    ...
    /// @dev Reserved time for users to send full withdraw priority operation in case of an upgrade (in seconds)
    uint256 internal constant MASS_FULL_WITHDRAW_PERIOD = 5 days;

    /// @dev Reserved time for users to withdraw funds from full withdraw priority operation in case of an upgrade (in seconds)
    uint256 internal constant TIME_TO_WITHDRAW_FUNDS_FROM_FULL_WITHDRAW = 2 days;

    /// @dev Notice period before activation preparation status of upgrade mode (in seconds)
    /// @dev NOTE: we must reserve for users enough time to send full withdraw operation, wait maximum time for processing this operation and withdraw funds from it.
    uint256 internal constant UPGRADE_NOTICE_PERIOD =
        $(
            defined(UPGRADE_NOTICE_PERIOD)
                ? UPGRADE_NOTICE_PERIOD
                : MASS_FULL_WITHDRAW_PERIOD + PRIORITY_EXPIRATION_PERIOD + TIME_TO_WITHDRAW_FUNDS_FROM_FULL_WITHDRAW
        );
